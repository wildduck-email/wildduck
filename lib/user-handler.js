...
class UserHandler {
    ...
    async delete(user, options) {
        ...
        if (existstingTask) {
            if (existstingTask.locked && existstingTask.status === 'pending') {
                // can update scheduled time
                let r = await this.database.collection('tasks').findOneAndUpdate(
                    { _id: existstingTask._id, locked: true, status: 'pending' },
                    {
                        $set: {
            ...
            let insRes = await this.database.collection('tasks').insertOne({
                task: 'user-delete',
                locked: options.deleteAfter ? true : false,
                lockedUntil: deleteAfter,
                created: now,
                status: options.deleteAfter ? 'pending' : 'queued',
                user
            });
    ...
    async restore(user, options) {
        ...
        if (existstingTask) {
            ...
            if (existstingTask.status !== 'pending') {
