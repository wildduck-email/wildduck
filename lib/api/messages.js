...
const snoozeQueue = new Queue('snoozeQueue', {
	host: db.redisConfig.host,
	port: db.redisConfig.port
});

module.exports = (db, server, messageHandler, userHandler, storageHandler, mailboxHandler) => {
     ...
     * @apiSuccess {Object} metaData Custom metadata object set for this message
        ...
        tools.asyncifyJson(async (req, res, next) => {
            ...
            let response = {
                ...
                references: (parsedHeader.references || '')
                    ...
                    .split(/\s+/)
                    .filter(ref => ref),
                metaData: formatMetaData(messageData.meta.custom)
            };
    ...
    /**
     * @api {post} /users/:user/mailboxes/:mailbox/messages/:message/snooze Snooze a message for later
     * @apiName SnoozeMessage
     * @apiGroup Messages
     * @apiHeader {String} X-Access-Token Optional access token if authentication is enabled
     * @apiHeaderExample {json} Header-Example:
     * {
     ...
     * @apiParam {String} mailbox ID of the Mailbox
     * @apiParam {Number} message ID of the Message
     * @apiQuery {Number} The time duration (in milliseconds) to keep the message snoozed for
     *
     * @apiSuccess {Boolean} success Indicates successful response
     * @apiSuccess {Number} id ID of the Message
     * @apiSuccess {String} mailbox ID of the Mailbox
     ...
     *
     * @apiExample {curl} Example usage:
     *     curl -i -XPOST "http://localhost:8080/users/59fc66a03e54454869460e45/mailboxes/59fc66a03e54454869460e46/messages/1/snooze?duration=30000"
     *
     * @apiSuccessExample {json} Success-Response:
     *     HTTP/1.1 200 OK
     *     {
     ...
     */
    server.post(
        { name: 'messages', path: '/users/:user/mailboxes/:mailbox/messages/:message/snooze' },
        tools.asyncifyJson(async (req, res, next) => {
            const schema = Joi.object().keys({
                user: Joi.string()
                    .hex()
                ...
                message: Joi.number()
                    ...
                    .required(),
                duration: Joi.number()
                    .min(1000) // You gotta snooze it for at least 1 second
                    .required()
            });
            ...
            let now = Date.now();
            let messageQuery = Number(message);

            // Check if [Snoozed] mailbox exist
            let mailboxes = await db.database
                    ...
                    .toArray();
            let exists = false;
            let snoozedMailbox = null;

            for(let i = 0; i < mailboxes.length; i++) {
                if(mailboxes[i].path === '[Snoozed]') {
                    exists = true;
                    snoozedMailbox = mailboxes[i]._id;
                    break;
                }
            }

            if(exists !== true) {
                // [Snoozed] mailbox doesn't exist. Create it.
                try {
                    let data = await createMailbox(user, '[Snoozed]', {
                        specialUse: true
                    });
                    snoozedMailbox = data.id;
                } catch (err) {
                    res.json({
                        error: err.message,
                        code: err.code
            ...
            try {
                let data = await moveMessage({
                    user,
                    source: { user, mailbox },
                    destination: { user, snoozeMailbox: snoozedMailbox }, // Move it to the [Snoozed] folder. EDIT: it works! yay!
                    messageQuery
                });
                info = data.info;
            } catch (err) {
            ...
            if (!info || !info.destinationUid || !info.destinationUid.length) {
                res.json({
                    error: 'Could not snooze message, check if message exists',
                    code: 'MessageNotFound'
                });
                return next();
            }

            await snoozeQueue.add('snooze', {
                task: 'unsnooze',
                user,
                messageId: messageQuery,
                originalMailbox: mailbox,
                snoozedMailbox,
                unsnoozeAt: now + duration
            }, {
                delay: duration
            });

            // Is this really needed anymore? Considering all tasks are going to be rewritten with bullmq?
...
function formatMessageListing(messageData) {
    ...
    if (messageData.meta && 'custom' in messageData.meta) {
        response.metaData = formatMetaData(messageData.meta.custom);
    }
